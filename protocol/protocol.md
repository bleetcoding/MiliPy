...
- **Capability:** a feature flag reported by the bridge during handshake. In protocol v1 it is a boolean; since the v1.1 extension it MAY be a rich object with a `state` field — see §9. A capability that is not `"available"` means an action targeting that feature must fail with `UnsupportedCapabilityError` on the SDK side.
...
```json
{
  ...
  "capabilities": {
    "screen_capture": true,
    "gesture_input": {"state": "available", "validated_on_device": false},
    "player_tracking": {"state": "unavailable"},
    "chat": {"state": "permission_required"},
    "settings_read": false,
...
| `capabilities` | Feature flags — the authoritative list of what this bridge build can actually do. Each value is either a boolean (v1) or a rich status object `{"state": "<state>", "validated_on_device": false}` (v1.1 extension). Valid states: `available`, `unavailable`, `permission_required`, `unsupported`, `not_validated`. The SDK must treat any boolean as `available` when true / `unavailable` when false. |
...
Responses to actions that include a `request_id` or `id`:
...
An `ack` carries `"status": "accepted"` when the bridge applied the action and `"rejected"` when it declined (unknown action, unsupported capability, malformed payload). `request_id` and `id` may appear together or separately — `request_id` is the legacy pairing field from v1; `id` is the action-level identifier introduced in the v1.1 extension. A client SHOULD set `id` to a unique string such as `"action-123"`; the bridge echoes whatever it receives and never fabricates one.
...
`request_id` and `id` are opaque strings generated by the SDK. If present, the bridge replies with `ack`, `result`, or `error` carrying the same identifier. Either field may be omitted (fire-and-forget actions).
...
`frame_rate` is an integer in `[0, 30]`; `jpeg_quality` is an integer in `[40, 95]`. Values outside these bounds are rejected with a structured `error` — the bridge never silently clamps a client's intent. The bridge's outbound frame queue is bounded (latest-frame semantics): when the client reads slower than the capture rate, stale frames are dropped and the most recent one is delivered, so a slow client never accumulates an unbounded backlog.
...
### 7.1 Rich capability status (v1.1 extension)

Capabilities MAY be reported as rich status objects rather than booleans:
